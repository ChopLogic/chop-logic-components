...
| **options**      | `ChopLogicSelectValue[]`                    | An array of objects (each object should contain two required fields - `id: string` and `label: string`) representing the options available in the Select dropdown. | `[]`             | Yes      |
| **onChange**     | `(values?: ChopLogicSelectValue[]) => void` | A function that is called when an option is selected. It can be used to handle the selection event.                                                                | `undefined`      | No       |
