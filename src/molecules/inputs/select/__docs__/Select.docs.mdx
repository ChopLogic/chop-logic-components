...
<Meta of={Select} title='ChopLogicSelect' />

# ChopLogicSelect

A styled and accessible select component that can be used both inside a form and as standalone controller.
...
| **onChange**     | `(value?: ChopLogicSelectValue) => void`    | A function that is called when an option is selected. It can be used to handle the selection event.                                                                | `undefined`      | No       |
