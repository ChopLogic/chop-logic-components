...
describe('TabContent', () => {
  ...
  it('renders extended tab content when extendable is true and no content found', () => {
    ...
    render(<TabContent {...defaultProps} selectedTabId='nonexistent-tab' extendable={true} extendedTabContent={extendedContent} />);
  ...
  it('renders regular content when extendable is true but content exists', () => {
    ...
    render(<TabContent {...defaultProps} selectedTabId='tab2' extendable={true} extendedTabContent={extendedContent} />);
  ...
  describe('Extended Tab Content Scenarios', () => {
    it('uses extended content when tab exists but has no content property', () => {
      ...
      render(<TabContent tabs={tabsWithMissingContent} selectedTabId='tab2' extendable={true} extendedTabContent={extendedContent} />);
    ...
    it('uses extended content when tab content is null', () => {
      ...
      const tabsWithNullContent: ChopLogicTabItem[] = [{ id: 'tab1', title: 'Tab 1', content: null as any }];

      const extendedContent = <div data-testid='extended-content'>Default Content</div>;

      render(<TabContent tabs={tabsWithNullContent} selectedTabId='tab1' extendable={true} extendedTabContent={extendedContent} />);
    ...
    it('uses extended content when tab content is undefined', () => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const tabsWithUndefinedContent: ChopLogicTabItem[] = [{ id: 'tab1', title: 'Tab 1', content: undefined as any }];

      const extendedContent = <div data-testid='extended-content'>Default Content</div>;

      render(<TabContent tabs={tabsWithUndefinedContent} selectedTabId='tab1' extendable={true} extendedTabContent={extendedContent} />);
